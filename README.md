...
- Think about to run tests concurrently, by now they run in one thread and I use the same user among the tests. I think this can be achieved by creating new users for each test with some special permissions (e.g. such users can see only projects and build that they create).
- Extend the number of fields that I use in payloads for requests and assert in responses. By far I use mostly necessary fields.
